...
function copyDotFiles() {
  unison "${HOME}"/Dev/DotfilesForMechonoid/"${REMOTE_IP}" ssh://"${USER}"@"${REMOTE_IP}"//home/"${USER}" -path .mechonoid -sshcmd "ssh.exe" -auto
}
...
if unison "${UNISON_ARGUMENTS[@]}"; then
  ...
  if [[ -d "${HOME}"/Dev/Mechonoid/node/node_modules/ ]]; then
    scp.exe -r "${USER}"@"${REMOTE_IP}":/home/"${USER}"/Mechonoid/node/node_modules/pigpio "${HOME}"/Dev/Mechonoid/node/node_modules/ >/dev/null
  fi
